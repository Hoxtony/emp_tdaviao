		...
		while true do
				...
				for index, values in pairs(startingPoints) do
					...
					if distance <= 3.0 then
						if vehicles[index] then
									...
									if emP.hasPermission() then
										if not emP.hasCooldown() then
											if not emP.isHappening() then
												...
												TriggerEvent('TDANotify', '~o~TDA ~s~| Despiste a policia enquanto o radar é desativado.')
											else
												TriggerEvent('TDANotify', '~r~Aguarde a entrega em andamento terminar.')
											end
										else
											TriggerEvent('TDANotify', '~r~Não temos entregas pra voce no momento, volte mais tarde.')
										end
									end
								end
							else
							...
							drawHelpTxt('~s~Nenhum avião disponivel, volte mais tarde!')
						end
					end
				end
			end
			...
			if happening == true then
				if not DoesEntityExist(empVehicle) then
					...
					if seconds > 0 then
						drawTxt(' O Radar está sendo desativado: <FONT color="#8de090">' .. math.floor(seconds / 60) .. ' minutos e ' .. math.floor(seconds % 60) .. ' segundos ~s~restantes', 6, 0.16, 0.87, 0.5, 255, 255, 255, 255)
					end

					if seconds <= 0 then
						drawTxt('Radar: <FONT color="#e08d8d">desativado', 6, 0.16, 0.87, 0.5, 255, 255, 255, 255)
						drawTxt('Vá até o local marcado no mapa e entregue o avião com as drogas.', 6, 0.16, 0.890, 0.45, 255, 255, 255, 120)
					end
